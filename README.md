...
I sometimes find that if the DRO has not been used for a while the calipers don't seem to power up in the correct state which can result in one or more of them not responding, if you turn the DRO off for a few seconds and back on this usually resolves the issue.  I guess they are not designed to have the battery removed, this could be resolved by giving the DRO control of the power to the capipers so it can power cycle them at startup, in fact they draw so little power that a gpio pin could probably power them directly.  Another option which may be worth considering is just power the calipers from a AA battery and leave them powered all the time.

If you enable showing of caliper read errors it is normal to get a read error every few seconds, in fact it tends to give erors at regular intervals which I suspect is caused by interrups breaking the timing of the caliper read, I just included this option as it can be useful when fault finding.

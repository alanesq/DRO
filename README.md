...
I have found that the power supply is critical for the calipers in that most switch mode USB power supplies seem to cause them to give random readings (although surprisingly when powered from my laptop they are fine), often the reading will just start incrementing for no apparent reason or change wildly.  I have made a linear power supply for mine now which is ok although the calipers still seem to bounce around a bit (varing by 1 or 2 hundredths of a mm).  Some kind of filter may be an option but I get the impression trying to filter the noise on a switched mode power supply is very difficult and the best option may be to just power the calipers from a battery as a single AA would probably power the three calipers for a year as they are designed to run from a watch battery.
